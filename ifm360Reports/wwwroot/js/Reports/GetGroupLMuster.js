...
const DaysName = ['Sunday', 'Monday', 'Tuesday', 'Wednesday', 'Thursady', 'Friday',
...
function SearchData() {
    ...
    $.ajax({
        ...
        url: localStorage.getItem('Myurl') + '/Reports/GetGroupLMuster',
        type: 'get',
        data: {
        ...
        success: function (data) {
            ...
            if (data.statusCode == 200) {
                ...
                for (let i = 1; i <= noOfDays; i++) {
                    ...
                    header1 += `<th class='days'><div>${DaysName[date.getDay()]}</div> 

</th>`
                    header += `<th class='days'><div>${i}-${months[tempmonth]}-${Year}
                ...
                for (var i = 0; i < data.length; i++) {
                  ...
                  <td>${data[i].EmpCode}</td>
                    ...
                    for (let j = 1; j <= noOfDays; j++) {
                        ...
                        let val = data[i][j];
                        row += `<td style="background-color:${val == 'A' ? '#F44336' : val

                            == 'P' ? '#4CAF50' : val == 'L' ? '#FFCDD2' : val == 'HD' ? '#AA73FF' : val

                                == 'R' ? '#FFBE00' : val == 'H' ? '#00BCD4' : val == 'OD' ?

                                    '#2196F3' : val == 'POD' ? '#9c27b0' : val == 'PR' ? '#fd7e14' : val == 'PL' ? '#d3e605' :
...
async function exportExcel() {
    ...
    for (let r = 0; r < table.rows.length; r++) {
        ...
        row.eachCell((cell, colNumber) => {
            ...
            if (r === 0 || r === 1) {
                ...
                else if (val === '' || val === "SNo" || val === "EmpCode" || val ===

                    "EmpName" || val === "DutyDays" || val === "Customer" || val == "Site") {
    ...
    saveAs(new Blob([buf]), `GroupL-Muster-${month}-2025.xlsx`);
}
