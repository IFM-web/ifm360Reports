...
async function exportExcel() {
    ...
    for (let r = 0; r < table.rows.length; r++) {
        ...
        row.eachCell((cell, colNumber) => {
            ...
            if (r === 0 || r === 1) {
                ...
                else if (val === '' || val === "SNo" || val === "EmpCode" || val === "EmpName" || val === "Duty Days") {
    ...
    saveAs(new Blob([buf]), `Attendance-Muster-ClientWise-${month}-2025.xlsx`);
}
...
function bindsite() {
    $.ajax({
        url: localStorage.getItem('Myurl') + '/Reports/bindSite',
        type: 'post',
        data: { siteid: $('#ddlClient').val() },
        success: function (data) {
            var data = JSON.parse(data);
