...
function showList(type) {
    $(".preloaderDiv").show();
    $.ajax({
        url: myurl + '/RnMReport/GetServiceslist',
        type: "get",
        data: {
...
function show(visitdate,customer,site) {
    $(".preloaderDiv").show();
    $.ajax({
        url: myurl + '/RnMReport/GetSoftServices',
        type: "get",
        data: {
            Customer: customer,
            site: site,
        ...
        success: (data) => {
            ...
            if (dt3.length > 0) {
                ...
                for (const section of allSections) {
                    ...
                    row += `
        <tr>
            <th colspan="8" style="background-color:#c84143; text-align:center;">${section}</th>
        </tr>
    `;
                     var image=""   
